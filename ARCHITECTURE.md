...
- 提供登录页和签名会话 Cookie 保护的页面与接口：
  ...
  - `/api/sms-config` – 获取或更新运行时短信供应商配置
...
- 模板和认证信息可以通过环境变量注入（`SMS_*`），也可以通过后台页面写入 `sms_config.json`
...
- 运行时短信配置会覆盖同名环境变量；`sms_config.json` 属于本地运行时文件，不应提交到仓库
...
- 可在文本框中编辑 `config.txt`，可视化配置短信供应商，并查看当前证书状态表格
- 通过 `fetch` 调用后台接口
...
4. 前端调用 `/api/sms-config` 读取短信供应商配置并展示配置状态
5. 每个域名通过 `/api/checkerSSLCertificate` 获取证书有效信息
6. 后端定时任务每日检查所有域名，在证书即将过期时使用当前短信配置发送通知
